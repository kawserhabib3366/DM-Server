...
def extractjsondata():
    ...
    with open("messages.txt", "a", encoding="utf-8") as f:
        for convo in data:
            ...
            # Extract analysis email and phone number
            analysis = convo.get("analysis", {})
            data_collection = analysis.get("data_collection_results", {})

            email_value = data_collection.get("email", {}).get("value")
            phone_value = data_collection.get("phonenumber", {}).get("value")

            if email_value:
                f.write(f"Email: {email_value}\n")
            if phone_value:
                f.write(f"Phone: {phone_value}\n")

            f.write("=========================================\n\n")

    print("Messages, email, and phone appended to messages.txt")
